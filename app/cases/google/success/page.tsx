...
export default function GoogleSuccessListPage() {
  return (
    <main className="relative min-h-screen bg-[#080c14]">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-20 left-1/3 h-[500px] w-[500px] rounded-full bg-emerald-600/5 blur-[120px]" />
        <div className="absolute bottom-0 right-1/4 h-[400px] w-[400px] rounded-full bg-indigo-600/4 blur-[100px]" />
      </div>
      ...
      <header className="sticky top-0 z-20 border-b border-white/5 bg-[#080c14]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-4xl items-center px-6 py-3.5">
          <nav className="flex items-center gap-1.5 text-xs text-gray-600">
            ...
            <span>구글 SEO</span>
            <span className="text-white/10">/</span>
            <span className="text-emerald-400 font-medium">성공</span>
          </nav>
        </div>
      </header>
      ...
      <div className="relative mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="mb-10 flex items-end justify-between">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="rounded-full border border-emerald-500/20 bg-emerald-500/8 px-3 py-0.5 text-[11px] font-bold text-emerald-400">
                구글 SEO · 성공
              </span>
            </div>
            <h1 className="text-3xl font-extrabold text-white">구글 성공 케이스</h1>
            <p className="mt-1.5 text-sm text-gray-500">
              Search Console API 실시간 연동 · {googleSuccessCases.length}개 사이트
            </p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {googleSuccessCases.map((c) => (
            <Link
              key={c.slug}
              href={`/cases/google/success/${c.slug}`}
              className="group relative overflow-hidden rounded-2xl border border-white/8 bg-white/[0.025] shadow-xl transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/25 hover:bg-white/[0.04] hover:shadow-2xl hover:shadow-emerald-950/30"
            >
              {/* 상단 라인 */}
              ...
              <div className="p-6">
                ...
                <div className="mt-5 flex items-center justify-between border-t border-white/5 pt-4">
                  ...
                  <span className="flex items-center gap-1 text-xs font-medium text-emerald-400 group-hover:gap-2 transition-all">
